...
〜をできるようにした（make it possible, enable to, allow）系のコミットメッセージ

> Enable to run the build script from any directory

> Allow build.sh to be run from a different directory

> Make build.sh runnable from any where

> Allow executing build.sh from any directory

直した（fix）系のコミットメッセージ

> Fix build.sh to work

> Fix build.sh so that it can be executed from any location

> Fix build.sh to be run properly when calling from another directory

> Fix the build script to make it executable

> Fix bugs of build.sh and make it runnable from any directory

- **Fix A to work** A が正常に動くように修正
- **Fix A so that ...** 〜になるように A を修正
- **Fix A to make it ...** A が 〜 できるように修正

ゴール記述系のコミットメッセージ

> Now build.sh can be executed from any directory


## 用例集

[gitにおけるコミットログ/メッセージ例文集100](http://anond.hatelabo.jp/20160725092419)の転載
